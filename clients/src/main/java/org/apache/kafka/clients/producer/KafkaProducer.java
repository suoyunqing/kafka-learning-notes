...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            //根据配置的partitioner_class，利用反射创建一个对象，并强转成Partitioner类；
            ...
            this.accumulator = new RecordAccumulator(logContext,
                    ...
                    apiVersions,
                    transactionManager,
                    //totalMemorySize内存池大小，BATCH_SIZE_CONFIG内存池中batch对象的最小值；
                    new BufferPool(this.totalMemorySize, config.getInt(ProducerConfig.BATCH_SIZE_CONFIG), metrics, time, PRODUCER_METRIC_GROUP_NAME));
            //kafka地址
            List<InetSocketAddress> addresses = ClientUtils.parseAndValidateAddresses(
                    config.getList(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG),
                    config.getString(ProducerConfig.CLIENT_DNS_LOOKUP_CONFIG));
            //获取元数据，包含两部分，Producer的元数据和kafka broker的数据；
            if (metadata != null) {
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
       ...
       //请求超时时间，默认30s；
        ...
        KafkaClient client = kafkaClient != null ? kafkaClient : new NetworkClient(
                ...
                throttleTimeSensor,
                logContext);
        //ack=0, 生产者发出，无需应答；=0，分区的leader broker收到，应答；=all或-1，ISR全部收到，应答；
    ...
    private TransactionManager configureTransactionState(ProducerConfig config,
        ...
        if (config.getBoolean(ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG)) {
            ...
            //如果transactionalId不为空，则为transactional producer;
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            //将消息发送至accmulator后，就视为发送完成，返回一个Future结果
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long nowMs, long maxWaitMs) throws InterruptedException {
        ...
        //无限循环直到拉取到目标topic的元数据或者超时，但这个步骤实际上并没有真正地拉取，而是取metadata中的topic数据
        do {
            if (partition != null) {
